...
3. **Read the sources.** Consult the full paper and captions, and check the authors' official project page and available supplementary material. Read relevant appendices/supplementary PDFs and inspect author explanation/demo videos when accessible; these can clarify the high-level idea, workflow, and qualitative behavior. Follow the companion-source guidance in [references/notebook-workflow.md](references/notebook-workflow.md), including source versions and video timestamps. Use primary sources for earlier methods. Retrieve official code or version-matched LaTeX source when it resolves a real ambiguity; rendered PDF/HTML remains the reference for the published layout. Record unavailable material and unresolved details rather than filling gaps from memory.
